#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Sticker renaming mapping generator"""
...
def main():
    ...
    for old_path, new_path in mappings.items():
        dir_name = os.path.basename(os.path.dirname(old_path))
        if dir_name not in by_character:
            by_character[dir_name] = []
        by_character[dir_name].append((old_path, new_path))

    # Print mappings by character
    for char in sorted(by_character.keys()):
        print(f"\n{'='*60}")
        print(f"  {char.upper()}")
        print(f"{'='*60}")
        for old_path, new_path in by_character[char]:
